...
module.exports = {
  ...
  host: function(address, cb) {
    dns.reverse(address, (err, hostnames) => {
      if (err) {
        console.error(
          chalk.red.bold(
            "Oh noes! Something went wrong. Maybe the IP",
            address,
            "is not valid?"
          )
        );
        ...
        console.log(
          chalk.blue.bold("Found", hostnames.length, "Hostname(s) for", address)
        );
  ...
  resolveRecord: function(domain, recordType, cb) {
    dns.resolve(domain, recordType, (err, addresses) => {
      if (err) {
        console.error(
          chalk.red.bold(
            "Oh noes! Something went wrong. Maybe the domain",
            domain,
            "does not exsist?"
          )
        );
        console.error(
          chalk.red.bold(".. Or, it does not have the requested record")
        );
        this.doCallback(cb);
      } else {
  ...
  displayRecords: function(domain, recordType, data, cb) {
    console.log(
      chalk.blue.bold("Found", data.length, recordType, "Record(s) for", domain)
    );
